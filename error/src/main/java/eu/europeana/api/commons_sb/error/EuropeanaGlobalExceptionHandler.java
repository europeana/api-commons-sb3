package eu.europeana.api.commons_sb.error;
...
import eu.europeana.api.commons_sb.error.config.ErrorConfig;
import eu.europeana.api.commons_sb.error.exceptions.ApplicationAuthenticationException;
import eu.europeana.api.commons_sb.error.exceptions.InvalidBodyException;
...
import static eu.europeana.api.commons_sb.error.config.ErrorConfig.BEAN_I18nService;
...
public class EuropeanaGlobalExceptionHandler {
    ...
    @Value("${spring.web.error.include-stacktrace:ON_PARAM}")
    private ErrorProperties.IncludeAttribute includeStacktraceConfig;
    ...
    protected boolean stackTraceEnabled(){
        return includeStacktraceConfig != ErrorProperties.IncludeAttribute.NEVER;
    }
    ...
    protected String buildResponseMessage(Exception e, String i18nKey, List<String> i18nParams) {
        if (i18nService != null && StringUtils.isNotBlank(i18nKey)) {
            if (i18nParams != null && !i18nParams.isEmpty()) {
